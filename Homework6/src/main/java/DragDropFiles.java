...
public class DragDropFiles extends JFrame {
    ...
    private AmazonS3 s3 = getDefaultS3();//= AmazonS3ClientBuilder.defaultClient();
    private int count = 0;
    private JTree tree;
    ...
    private static AmazonS3 getDefaultS3()
    {
   
    	BasicAWSCredentials creds = new BasicAWSCredentials("aws-access-key", "aws-secret-key");
		AmazonS3 s3 = AmazonS3ClientBuilder.standard().withCredentials(new AWSStaticCredentialsProvider(awsCreds)).withRegion("us-west-2").build();
    	
    	return s3;
    	
    }

    private static DefaultTreeModel getS3TreeModel(AmazonS3 s3)
    {
    	DefaultMutableTreeNode root;
    	if(!s3.doesBucketExistV2("base"))
    	{
    		root = new DefaultMutableTreeNode(s3.createBucket("base").getName());
    	}
    	else
    	{
    		...
    		for(Bucket b: buckets)
    		{
    			...
    			{
    				child.add(new DefaultMutableTreeNode(summary.getKey()));
    			}
    		}
    	}
    	return new DefaultTreeModel(root);
    }//end of getS3TreModel
    ...
    public DragDropFiles() {
        ...
        treeModel = getS3TreeModel(this.s3);
            ...
            public boolean importData(TransferHandler.TransferSupport info) {            	
                ...
                try {
                    ...
                    for (File f : l) {
                    		...
                    		//copyFile(f, destFile);
                    		s3.putObject(new PutObjectRequest(path.getLastPathComponent().toString(), uploadName, f));
        ...
        download.addActionListener(new ActionListener() { 
        	  public void actionPerformed(ActionEvent e) { 
        	    //TODO:You have to program here in this method in response to downloading a file from the cloud,
        	    ...
        	    if(downloadPath != null) 
        	    {
        	    		...
        	    		String filename = downloadPath.getPath()[downloadPath.getPathCount() -1].toString();
        	    		S3Object object = s3.getObject(new GetObjectRequest(downloadPath.getPath()[downloadPath.getPathCount() -2].toString(), filename));
        	    	InputStream objectData = object.getObjectContent();
        	    	label.setText("Succesfully downloaded a file from cloud from buckets:" + 
        	    				downloadPath.toString());
        	    	try
					{
        	    		String[] array = filename.split("./");
						IOUtils.copy(objectData, new FileOutputStream(new File("./" + array[array.length - 1])));
        ...
        delete.addActionListener(new ActionListener()
        		{
        			public void actionPerformed(ActionEvent e)
        			{
        				int confirm = JOptionPane.showConfirmDialog(null, "Do you want to delete \""+ downloadPath.toString() + "\" ?");
        				if(confirm == 0)
        				{
        					String filename = downloadPath.getPath()[downloadPath.getPathCount() -1].toString();
        					String bucketName = downloadPath.getPath()[downloadPath.getPathCount() -2].toString();
        					s3.deleteObject(new DeleteObjectRequest(bucketName, filename));
        					label.setText("Deleted " + filename);
        				}
